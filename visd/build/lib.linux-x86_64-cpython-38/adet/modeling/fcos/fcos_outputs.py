...
class FCOSOutputs(nn.Module):
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, offset_pred, locations, gt_instances, top_feats=None, top_feats2=None, top_feats3=None, top_feats4=None):
        """
        ...
        if len(top_feats3) > 0:
            instances.top_feats3 = cat([
                # Reshape: (N, -1, Hi, Wi) -> (N*Hi*Wi, -1)
                x.permute(0, 2, 3, 1).reshape(-1, x.size(1)) for x in top_feats3
            ], dim=0,)

        if len(top_feats4) > 0:
            instances.top_feats4 = cat([
                # Reshape: (N, -1, Hi, Wi) -> (N*Hi*Wi, -1)
                x.permute(0, 2, 3, 1).reshape(-1, x.size(1)) for x in top_feats4
            ], dim=0,)
    ...
    def predict_proposals(
            ...
            locations, image_sizes, top_feats=None, top_feats2=None, top_feats3=None, top_feats4=None
    ):
        if self.training:
        ...
        if len(top_feats3) > 0:
            bundle["t3"] = top_feats3
        if len(top_feats4) > 0:
            bundle["t4"] = top_feats4
        ...
        for i, per_bundle in enumerate(zip(*bundle.values())):
            ...
            t3 = per_bundle["t3"] if "t3" in bundle else None
            t4 = per_bundle["t4"] if "t4" in bundle else None

            sampled_boxes.append(
                self.forward_for_single_feature_map(
                    l, o, r, c,offset, image_sizes, t, t2, t3, t4
                )
            )
    ...
    def forward_for_single_feature_map(
            ...
            ctrness_pred, offset_pred, image_sizes, top_feat=None ,top_feat2 = None, top_feat3 = None, top_feat4 = None
    ):
        ...
        if top_feat3 is not None:
            top_feat3 = top_feat3.view(N, -1, H, W).permute(0, 2, 3, 1)
            top_feat3 = top_feat3.reshape(N, H * W, -1)

        if top_feat4 is not None:
            top_feat4 = top_feat4.view(N, -1, H, W).permute(0, 2, 3, 1)
            top_feat4 = top_feat4.reshape(N, H * W, -1)

        # if self.thresh_with_ctr is True, we multiply the classification
        # scores with centerness scores before applying the threshold.
        '''
        if self.thresh_with_ctr:
            #logits_pred = torch.cat(((logits_pred[:,:,0]*ctrness_pred).unsqueeze(2), logits_pred[:,:,1].unsqueeze(2)),2)
            logits_pred = logits_pred * ctrness_pred[:, :, None]
        '''

        candidate_inds = logits_pred > self.pre_nms_thresh #0.0005
        ...
        if not self.thresh_with_ctr:
            logits_pred = logits_pred * ctrness_pred[:, :, None]
            # logits_pred = torch.cat((logits_pred[:, :, 0].unsqueeze(2), (logits_pred[:, :, 1]* ctrness_pred).unsqueeze(2)), 2)
        '''
        ...
        for i in range(N):
            ...
            per_ctrness_pred = ctrness_pred[i]
            per_ctrness_pred = per_ctrness_pred[per_box_loc]
            ...
            if top_feat3 is not None:
                per_top_feat3 = top_feat3[i]
                per_top_feat3 = per_top_feat3[per_box_loc]
            if top_feat4 is not None:
                per_top_feat4 = top_feat4[i]
                per_top_feat4 = per_top_feat4[per_box_loc]
            ...
            if per_candidate_inds.sum().item() > per_pre_nms_top_n.item():
                ...
                per_ctrness_pred = per_ctrness_pred[top_k_indices]
                if top_feat is not None:
                    per_top_feat = per_top_feat[top_k_indices]
                if top_feat2 is not None:
                    per_top_feat2 = per_top_feat2[top_k_indices]
                if top_feat3 is not None:
                    per_top_feat3 = per_top_feat3[top_k_indices]
                if top_feat4 is not None:
                    per_top_feat4 = per_top_feat4[top_k_indices]
            ...
            boxlist.ctrness_pred = per_ctrness_pred
            if top_feat is not None:
                boxlist.top_feat = per_top_feat
            if top_feat2 is not None:
                boxlist.top_feat2 = per_top_feat2
            if top_feat3 is not None:
                boxlist.top_feat3 = per_top_feat3
            if top_feat4 is not None:
                boxlist.top_feat4 = per_top_feat4
