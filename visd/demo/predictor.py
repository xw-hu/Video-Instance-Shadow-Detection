    ...
    def run_on_image(self, image):
        ...
        if "panoptic_seg" in predictions:
            ...
            if "instances" in predictions:
                ...
                instances.track_embedding = instances.track_embedding.numpy()

                # Hi man, please check ur 'mask_ann'. For me, some annotations only have 3 or 2 points, which I called bad segmentations
                # https://github.com/open-mmlab/mmdetection/issues/3338
                for index, pred_mask in enumerate(instances.pred_masks):
                    # print(np.sum(pred_mask), pred_mask.shape)
                    if np.sum(pred_mask) < 4:
                        instances.pred_masks[index][0][0] = 1
                        instances.pred_masks[index][0][1] = 1
                        instances.pred_masks[index][0][2] = 1
                        instances.pred_masks[index][0][3] = 1
